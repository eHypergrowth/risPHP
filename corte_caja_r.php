...
<table border="0" cellspacing="0" id="dataTable" width="90%" height="100%" class="table-condensed table-striped table-hover">
    <thead>
      <tr class="bg-primary">
        <th width="92px" id="clickme" style="white-space:nowrap;">REFERENCIA</th>
        <th>PACIENTE</th>
    ...
    <tfoot>
		<tr class="bg-primary">
			...
			<th><input type="text" value="¿?" maxlength="8" class="" /></th>
...
$(document).ready(function(e) {
	//breadcrumb
	$('#breadc').html('<li><a href="index.php">HOME</a></li><li>RECEPCIÓN</li><li class="active"><strong>CORTE DE CAJA</strong></li>');
	...
	var tam = $('#referencia').height() - 244 - $('#breadc').height(), wT = $('#referencia').width()*0.9;
	...
	var oTable = $('#dataTable').dataTable({
		...
		"fnFooterCallback": function ( nRow, aaData, iStart, iEnd, aiDisplay ) {
			...
			$('.opcTable').css('float','left'); $('span.DataTables_sort_icon').remove(); $("#saldos").button();
			...
			nCells[4].innerHTML = '$'+redondear(parseFloat(iSubT * 100)/100,2);
			nCells[5].innerHTML = '$'+redondear(parseFloat(iCargosE * 100)/100,2);
		...
		"aaSorting": [[3, "desc"]], "iDisplayLength": 500000000000000, "bLengthChange": false, "bProcessing": false, "bServerSide": true,
		...
		"oTableTools": {
			...
			"aButtons": [ 
				...
				{ "sExtends": "xls", "sButtonText": "EXCEL" }, { "sExtends": "copy", "sButtonText": "COPIAR" },
				{ "sExtends": "print", "sButtonText": "IMPRIMIR", "sInfo": "</br>VISTA DE IMPRESIÓN </br></br></br> Presione SCAPE al terminar." }
			]
		},
		"sAjaxSource": "corte_caja/reportes/datatable-serverside/reportes.php",
		...
		"oLanguage": {
			"sLengthMenu": "_MENU_ REGISTROS", "sZeroRecords": "SIN COINCIDENCIAS - LO SENTIMOS", "sInfo": "MOSTRADOS: _END_",
			"sInfoEmpty": "MOSTRADOS: 0", "sInfoFiltered": "&nbsp;REGISTROS: _MAX_", "sSearch": "BUSCAR"
		}
	...
	$(document).delegate('#dataTable tbody td img','click', function () {
        ...
        { /* Open this row */
			...
			function processDataOV(dataOV) {
		  		console.log(dataOV); var text = dataOV.split('/'); oTable.fnOpen( nTr, fnFormatDetails(oTable, nTr, dataOV/*text[0], text[1], text[2]*/), 'details' );
       		} // end processDataOV
        }
    } );
...
function pagar(ref, tot, sal, abo, pac, noTemp, indicador, tCo, tIm, tLab, tFar){
	...
	$("#myModal").load("corte_caja/reportes/htmls/dialogPagarOV.php",function(response,status,xhr){if (status == "success"){
		...
		$('#pagaConP').keyup(function(e) {
			if( $(this).val() > parseFloat(montoPagar) ){
				$('#cambioP').val(redondear(parseFloat($('#pagaConP').val())-parseFloat(montoPagar),2) );
			}else{$('#cambioP').val('')}
		});
						...
						function(){
							...
							$("#myModal1").load('pacientes/htmls/ticket.php', function( response, status, xhr ){ if(status=="success"){
								...
								$.post('pacientes/files-serverside/datosTicket_a.php',datosTi).done(function(data){
									...
									$('#adicionalesT').text(datosT[10]);$('#totalT').text(datosT[4]);$('#pagoT').text(montoPagar);
									...
									$('#abonosT').text(redondear(parseFloat(datosT[30]),2)); $('#saldosT').text(redondear(-1*(parseFloat(datosT[31])),2));
									$('#myAbonadoA').removeClass('hidden'); //$('#contenedorAnteriores, #myAbonadoA').removeClass('hidden');
