...
<head>
    ...
    <title>SISTEMA - REPORTE DE PRODUCTIVIDAD DE RECEPCIÓN</title>
...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" id="dataTablePrincipal" class="table table-hover table-striped dataTables-example dataTable table-condensed" role="grid"> 
	<thead id="cabecera_tBusquedaPrincipal">
	  <tr role="row" class="bg-primary">
		...
		<th style="vertical-align:middle;" nowrap>USUARIO</th>
		<th style="vertical-align:middle;" nowrap>#ÓRDENES DE VENTA</th>
		<th style="vertical-align:middle;" nowrap>PORCENTAJE VENTAS</th>
		<th style="vertical-align:middle;" nowrap>NUEVOS PACIENTES</th>
		<th style="vertical-align:middle;" nowrap>PORCENTAJE PACIENTES</th>
	  </tr>
...
$(document).ready(function(e) {
	//breadcrumb
	$('#breadc').html('<li><a href="index.php">RECEPCIÓN</a></li><li class="active"><strong>REPORTE DE PRODUCTIVIDAD</strong></li>');
	...
	var oTableP = $('#dataTablePrincipal').DataTable({
		...
		"fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay){
			var iTotal = 0, iTotalP = 0;
            for(var i=iStart ; i<iEnd ; i++){ iTotal += aaData[ aiDisplay[i] ][2]*1; iTotalP += aaData[ aiDisplay[i] ][4]*1; }

            /* Modify the footer row to match what we want */
            var nCells = nRow.getElementsByTagName('th');
            nCells[2].innerHTML = redondear(parseFloat(iTotal * 100)/100,2); nCells[4].innerHTML = redondear(parseFloat(iTotalP * 100)/100,2);
		},
		...
		"sAjaxSource": "pacientes/datatable-serverside/productividad_rec.php",
