...
<head>
    ...
    <meta name="author" content="DIAGNOCONS">

    <title>SISTEMA - IMAGENOLOGÍA</title>
...
<!-- Contenido -->
<input id="t_id_s" type="hidden" value=""> <input id="t_clave_s" type="hidden" value=""> <input id="email_pacite" type="hidden" value=""> <input id="email_medico" type="hidden" value="">
	<div class="hidden" id="dpa_imprimir"></div><div class="hidden" id="dpa_imprimir1"></div> <input id="link_email" type="hidden" value="">
	<input id="paciente_email" type="hidden" value=""> <input id="estudio_email" type="hidden" value="">
...
<thead id="cabecera_tBusquedaPrincipal">
  <tr role="row" class="bg-primary">
    ...
    <th style="vertical-align:middle;">ACCESO</th>
	...
	<tfoot>
        <tr class="bg-primary">
            ...
            <th><input type="text" class="form-control input-sm" placeholder="-ESTATUS-" style="max-width:110px;"/></th>
            <th></th>
            <th></th>
            <th></th>
            <th><input type="text" class="form-control input-sm" placeholder="-ÁREA-" style="width:80%;"/></th>
            <th><input type="text" class="form-control input-sm" placeholder="-SUCURSAL-" style="width:90%;"/></th>
            <th></th>
        </tr>
    </tfoot>
</table>
...
$(document).ready(function(e) {
	//breadcrumb
	$('#breadc').html('<li><a href="index.php">HOME</a></li><li>IMAGEN</li><li class="active"><strong>ESTUDIOS DE IMAGENOLOGÍA</strong></li>');
	...
	var oTableP = $('#dataTablePrincipal').DataTable({
		bServerSide: true,"sScrollY": tamP, ordering: false, searching: true, scrollCollapse: false, "scrollX": true,
		...
		deferRender: true, select: false, "bProcessing": true,
		"sAjaxSource": "imagen/datatable-serverside/estudios.php",
		...
		},"iDisplayLength": 500000, paging: false,
	});
	...
	$('.infoPrincipal').append('<div id="divRangoFechas" style="text-align:left;"> <table width="100%" border="0" cellpadding="4" cellspacing="2" style="color:black; float:left;"> <tr> <td width="10px">De &nbsp;</td> <td width="1%"> <input name="fechaDe" class="fechas form-control input-sm" data-provide="datepicker" data-date-format="yyyy-mm-dd" style="width:100px;" type="text" id="fechaDe" value="<?php echo date("Y-m-d"); ?>" readonly > </td> <td width="10px">&nbsp; A &nbsp;</td> <td width="1%"> <input style="width:100px;" name="fechaA" type="text" class="fechas form-control input-sm" data-provide="datepicker" data-date-format="yyyy-mm-dd" id="fechaA" value="<?php echo date("Y-m-d"); ?>" readonly> </td> <td id="rangosFechas">&nbsp; <input type="radio" class="rad" id="radio1" name="radio" /><label for="radio1">&nbsp; Hoy &nbsp;</label> <input type="radio" class="rad" id="radio2" name="radio" /><label for="radio2">&nbsp; Todos &nbsp;</label> </td> </tr> </table> </div>');
...
function noest(idVC,nombreE,nombreP,ref,fecha, idPacs){
	$("#myModal1").load('imagen/htmls/asignar_pacs.php', function(response,status,xhr){if(status == "success"){
		...
		oTable.on( 'select', function(e, dt, type, indexes){
			...
			var rowData = oTable.rows( indexes ).data().toArray(); $('#pkPacs').val(rowData[0][5]); // alert(rowData[0][4]);
...
function est(a,b,idPacs){/*a es id del estudio, b es el id del estudio ambos en venta de conceptos*/
	...
	if(sodb[0]=='https' || sodb[0]=='HTTPS'){
		var myL = url.split('https://'), myL1 = myL[1].split('/'), koby = myL1[0].split(':80');
		var link_1 = koby[0]; //alert(link_1);
	}else{
		var myL = url.split('http://'), myL1 = myL[1].split('/'), koby = myL1[0].split(':80');
		var link_1 = koby[0]+koby[1]; // alert(myL1[0]);//alert(myL[0]);
	}

	var idE = {idE:a,h:koby[0],link:koby[0],id_pacs:idPacs}
	$.post('imagen/files-serverside/datosInterpretar.php',idE).done(function( data ) { // console.log(data);
		var dataE = data.split(';*}-{'); // alert(dataE[0]);
		if(dataE[0]!=''){
			if(isMobile){
				window.open('http://'+koby[0]+':80/sistema/viewer.php/viewer/'+dataE[34], '_self');
			}else{
				window.open('http://'+koby[0]+':80/sistema/viewer.php/viewer/'+dataE[34]);
				/* $("#myModal1").load('imagen/htmls/mi_estudio.php', function(response,status,xhr){if(status == "success"){
					$('#referencia_est').val(dataE[12]); $('#idEstudioPacs').val(dataE[19]); $('#email_pacite').val(dataE[36]); $('#email_medico').val(dataE[37]);
					$('#paciente_est, #paciente_email').val(dataE[0]); $('#folio_est, #estudio_email').val(dataE[8]);
					...
					var ka = dataE[17];
					$('#chosto').html(''); $('#btn_v_osirix').hide(); //$('#btn_v_osirix').replaceWith(ka);
					var mi_referCh = $('#referencia_est').val().replace(/([\ \t]+(?=[\ \t])|^\s+|\s+$)/g, '');
					// Para cuando hay server en sitio:
					$('#chosto').text('http://'+koby[0]+':8080/oviyam2/viewer.html?patientID='+mi_referCh+'&studyUID='+dataE[34]);
					$('#chosto1').text('http://'+koby[0]+':8080/weasis-pacs-connector/weasis?patientID='+mi_referCh+'&studyUID='+dataE[34]);

					$('#link_email').val('http://radiologiazaragoza.ddns.net:8080/weasis-pacs-connector/weasis?patientID='+mi_referCh+'&studyUID='+dataE[34]);

					$('#btn_v_html').click(function(e) {
						window.open('http://'+koby[0]+':8888/sistema_cabos_arc/viewer.php/viewer/'+dataE[34]);
					});

					$('#btn_v_avanzado').click(function(e) { });

					var linki = '"<a href="http://radiologiazaragoza.ddns.net:8080/weasis-pacs-connector/weasis?patientID="+mi_referCh+"&studyUID="+dataE[34]" class="btn btn-default">Launch</a>"';
					console.log(linki);

					$('#btn_v_avanzado').replaceWith(dataE[35]);
...
function previsualiza(uiud){
	...
	if(isMobile){
		window.open('http://'+koby[0]+'/sistema/viewer.php/viewer/'+uiud, '_self');
	}else{
		window.open('http://'+koby[0]+'/sistema/viewer.php/viewer/'+uiud);
	}
}

//ESTA PARTE ES LA QUE ENVIA EL ESTUDIO AL PACIENTE Y MEDICO REFERENTE Lindeb
function sendEmail(email_pac, email_us, estudio, paciente, patientID, studyUID){
	$('#myModal1').modal('hide'); patientID = patientID.replace(/([\ \t]+(?=[\ \t])|^\s+|\s+$)/g, '');

	var linki = 'http://ns1.diagnocons.com/sistema/viewer.php/viewer/'+studyUID;

	window.setTimeout(function(){
		$("#myModal3").load('imagen/htmls/mi_email.php', function(response,status,xhr){if(status == "success"){
			$('#alerta_form').hide(); // $('#email_pac').val($('#email_pacite').val()); $('#email_pac2').val($('#email_medico').val());
			$('#email_pac').val(email_pac); $('#email_pac2').val(email_us); var mensaje = 'DiagnoCons ha enviado un estudio de '+estudio+' del paciente '+paciente;
			$('#mensajeE').val(mensaje);

			/*----------------------ENVIO DE ESTUDIO POR CORREO ----------------------------------*/
			$('#send_mail').click(function(e){
				swal({title:"Espere un momento", text:"El correo se está mandando...", type:"info",showConfirmButton:false});

				var datos={emailPaciente:$('#email_pac').val(),link:linki,paciente:paciente,estudio:estudio,body:$('#mensajeE').val(),emailMedReferente:$('#email_pac2').val(),medicoRadiologo:$('#medicoRadiologo').val(),tecnicoEstudio:$('#tecnicoEstudio').val(),ordenImg:$('#ordenImg').val()}
				console.log(datos);

				$.post('imagen/files-serverside/send_mail.php',datos).done(function(data){
					if (data==1){
						$('#myModal3').modal('hide'); swal("Correo enviado", "El correo ha sido enviado correctamente.", "success");
						window.setTimeout(function(){ swal.close(); },2000);
					}else{
						swal("Correo no enviado", "El correo no pudo enviarse, favor de intentarlo nuevamente.", "error");
						window.setTimeout(function(){ swal.close(); },2000);
					}
				});
			});
			/*----------------------FIN ENVIO DE ESTUDIO POR CORREO ----------------------------------*/

			$('#myModal3').modal('show');
			$('#myModal3').on('shown.bs.modal', function (e) {
				$('#form-mail').validator(); $('#form-mail input').keyup(function(e) { $('#alerta_form').hide(); });
			});
